	...
	var ytEmbed = {
		...
		*/	
			...
			if(json.error){
				...
				for (var i = 0; i < json.data.items.length; i++) {
					...
					if(this.cfg.layout == 'thumbnails'){
						...
						//this.cfg.layout = full
					  li.innerHTML = '<table cellspacing="0" cellpadding="0" border="0"><tr><td valign="top" rowspan="2"></td><td valign="top"><b>'+entry.title+'</b><br/><span>'+this.formatDescription(entry.description).substring(0,100)+'</span></td></tr></table>';
		...
		createPlayer : function(cfg){
			...
			iframe.setAttribute('src', 'http://www.youtube.com/embed/'+ytPlayerParams.videoId+'?autoplay='+ytPlayerParams.autoplay+'&modestbranding=1&origin='+document.location.protocol+'//'+document.location.hostname);//controlbar set
